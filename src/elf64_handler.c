...
/*   Updated: 2024/10/24 22:44:23 by franmart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_nm.h"

void	process_elf64(void *map)
{
	...
	if (sh_sym && sh_str)
	{
		...
		extract_elf64_symbols(symtab, n_sym, n_sh, sh, strtab);
	}
	else if (!sh_sym)
...
void	extract_elf64_symbols(t_ELF64_symbol *symtab, uint64_t n_sym, uint16_t n_sh,
							t_ELF64_section_header *sh, char *strtab)
{
	...
	while (++i < n_sym)
		symbols[i] = &symtab[i];
	symbols = sort_elf64_symbols(symbols, n_sym, strtab);
	i = -1;
	while (++i < n_sym)
...
t_ELF64_symbol **sort_elf64_symbols(t_ELF64_symbol **list, uint64_t len, char *str_table)
{
	uint64_t	mid = len / 2;
	...
	t_ELF64_symbol **left = sort_elf64_symbols(list, mid, str_table);
	t_ELF64_symbol **right = sort_elf64_symbols(list + mid, len - mid, str_table);
